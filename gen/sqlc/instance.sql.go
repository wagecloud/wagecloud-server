...
func (q *Queries) DeleteInstance(ctx context.Context, arg DeleteInstanceParams) error {
	_, err := q.db.Exec(ctx, deleteInstance, arg.ID, arg.AccountID)
	return err
}
...
func (q *Queries) GetInstance(ctx context.Context, arg GetInstanceParams) (InstanceBase, error) {
	row := q.db.QueryRow(ctx, getInstance, arg.ID, arg.AccountID)
	var i InstanceBase
	err := row.Scan(
		&i.ID,
		&i.AccountID,
